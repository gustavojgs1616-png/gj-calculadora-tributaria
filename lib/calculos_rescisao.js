...
// ── Seguro-Desemprego (Resolução CODEFAT nº 995/2024 — vigente 2025/2026) ───
// Faixas para cálculo do valor da parcela
export const TABELA_SEGURO_DESEMPREGO = [
  { ate: 2041.39,  calcular: (s) => s * 0.80 },
  { ate: 3402.28,  calcular: (s) => 1633.11 + (s - 2041.39) * 0.50 },
  { ate: Infinity, calcular: ()  => 2314.75 }, // teto
];

// Número de parcelas por solicitação × faixa de meses trabalhados
// faixa 0 = 6–11 meses | faixa 1 = 12–23 meses | faixa 2 = 24+ meses
export const PARCELAS_SEGURO = {
  1: [4, 5, 5], // 1ª solicitação
  2: [3, 4, 5], // 2ª solicitação
  3: [3, 3, 3], // 3ª solicitação ou mais
};

/**
 * Calcula Seguro-Desemprego
 * @param {number} salario1  Remuneração bruta do mês -1
 * @param {number} salario2  Remuneração bruta do mês -2
 * @param {number} salario3  Remuneração bruta do mês -3
 * @param {number} mesesTrabalhados  Meses trabalhados nos últimos 36 meses
 * @param {number} solicitacao  1 | 2 | 3 (3 = 3ª ou mais)
 */
export function calcSeguroDesemprego({ salario1, salario2, salario3, mesesTrabalhados, solicitacao }) {
  const meses = Math.max(0, parseInt(mesesTrabalhados) || 0);
  const sol   = Math.min(Math.max(parseInt(solicitacao) || 1, 1), 3);

  // Elegibilidade mínima
  if (meses < 6) return { elegivel: false, motivo: "Mínimo de 6 meses trabalhados nos últimos 36 meses não atingido." };

  // Média dos últimos 3 salários
  const s1 = parseFloat(salario1) || 0;
  const s2 = parseFloat(salario2) || 0;
  const s3 = parseFloat(salario3) || 0;
  const salarioMedio = (s1 + s2 + s3) / 3;

  if (salarioMedio <= 0) return { elegivel: false, motivo: "Informe os salários dos últimos 3 meses." };

  // Número de parcelas
  const faixaMeses = meses >= 24 ? 2 : meses >= 12 ? 1 : 0;
  const parcelas = PARCELAS_SEGURO[sol][faixaMeses];

  // Valor da parcela
  let valorParcela = 0;
  for (const faixa of TABELA_SEGURO_DESEMPREGO) {
    if (salarioMedio <= faixa.ate) { valorParcela = faixa.calcular(salarioMedio); break; }
  }
  ...
  return {
    ...
    parcelas,
    valorParcela,
    totalReceber: valorParcela * parcelas,
    salarioMedio,
